...
def get_lower_upper(image, color_space):
    ...
    with mp_hands.Hands(static_image_mode=True, max_num_hands=1, min_detection_confidence=0.5) as hands:
        ...
        for position in hand_landmarks.landmark:
            x, y = position.x*image_width, position.y*image_height
            threshold.append(image_color_space[int(y), int(x)])
    threshold = np.array(threshold)
    lower = []
    upper = []
    for axis in range(3):
        lower.append(threshold[:, axis].min())
        upper.append(threshold[:, axis].max())
...
if __name__ == '__main__':
    ...
    # get lower and upper
    lower, upper = get_lower_upper(image=image, color_space=color_space)
