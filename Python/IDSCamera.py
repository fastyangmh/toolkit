...
class IDSCamera:
    def __init__(self, camera_parameter_path):
        ...
        self.config = self.load_ini_file(
        ...
        self.color_mode = self.color_modes_table[self.config['Parameters']
                                                 ['Colormode']]
        ...
        self.check(result=result, info='initial')

        # set color mode
        result = ueye.is_SetColorMode(hCam=self.camera,
                                      Mode=eval(self.color_mode))
        self.check(result=result, info='set color mode')

        # load parameter
        result = ueye.is_ParameterSet(
            hCam=self.camera,
            nCommand=ueye.IS_PARAMETERSET_CMD_LOAD_FILE,
            pParam=c_wchar_p(camera_parameter_path),
            cbSizeOfParam=c_uint(0))
        self.check(result=result, info='load parameter')
        ...
        result = ueye.is_AllocImageMem(hCam=self.camera,
                                       ...
                                       ppcImgMem=self.mem_ptr,
                                       pid=mem_id)
        self.check(result=result, info='allocate memory')

        # set active memory region
        result = ueye.is_SetImageMem(hCam=self.camera,
                                     pcMem=self.mem_ptr,
                                     id=mem_id)
        self.check(result=result, info='set active memory region')

        # set DIB
        result = ueye.is_SetDisplayMode(hCam=self.camera,
                                        Mode=ueye.IS_SET_DM_DIB)
        self.check(result=result, info='set DIB')

    def load_ini_file(self, camera_parameter_path):
    ...
    def __call__(self):
        ...
        self.check(result=result, info='freeze video')
        image = ueye.get_data(image_mem=self.mem_ptr,
                              x=self.width,
                              y=self.height,
                              bits=self.bitspixel,
                              pitch=self.lineinc,
    ...
    def release(self):
        ...
        self.check(result=result, info='exit camera')
