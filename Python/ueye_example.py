...
print(f"initCamera returns {ret}")  # success will get 0

# set color mode
ret = ueye.is_SetColorMode(hcam, ueye.IS_CM_BGR8_PACKED)
print(f"SetColorMode IS_CM_BGR8_PACKED returns {ret}")  # success will get 0
...
print(f"AOI IS_AOI_IMAGE_SET_AOI returns {ret}")    # success will get 0
...
ret = ueye.is_AllocImageMem(hcam, width, height, bitspixel,
                            mem_ptr, mem_id)
print(f"AllocImageMem returns {ret}")   # success will get 0

# set active memory region
ret = ueye.is_SetImageMem(hcam, mem_ptr, mem_id)
print(f"SetImageMem returns {ret}")  # success will get 0

# continuous capture to memory
ret = ueye.is_CaptureVideo(hcam, ueye.IS_DONT_WAIT)
print(f"CaptureVideo returns {ret}")    # success will get 0
...
print(f"StopLiveVideo returns {ret}")   # success will get 0
ret = ueye.is_ExitCamera(hcam)
print(f"ExitCamera returns {ret}")  # success will get 0
